...
description: Use this skill when another agent's expertise/context is needed, or when the user explicitly asks to involve another agent. First list agents, then use molili agents chat for two-way communication with replies. | 当需要其他 agent 的专长/上下文，或用户明确要求调用其他 agent 时使用；先查 agent，再用 molili agents chat 双向通信（有回复）
...
2. molili agents list
3. molili agents chat 发起对话
4. 从输出中记录 [SESSION: ...]
...
```bash
molili agents -h
molili agents list -h
molili agents chat -h
```
