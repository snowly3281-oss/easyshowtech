---
name: guidance
description: "回答用户关于 molili 安装与配置的问题：优先定位并阅读本地文档，再提炼答案；若本地信息不足，兜底访问官网文档。"
metadata:
  {
...
当用户询问 **molili 的安装、初始化、环境配置、依赖要求、常见配置项** 时，使用本 skill。
...
```bash
# 获取 memory 中的文档目录（支持新旧路径）
DOC_DIR=$(find ~/.molili/workspaces/default/memory/ ~/.molili/memory/ ~/.molili/memory/ -maxdepth 2 -type d -name "docs" 2>/dev/null | head -n 1)
```
...
```bash
# 获取二进制绝对路径
COP_PATH=$(which molili 2>/dev/null || whereis molili | awk '{print $2}')

# 逻辑推导：如果路径包含 .molili/bin/molili 或 .molili/bin/molili，则根目录在其上三层
if [[ "$COP_PATH" == *".molili/bin/molili" ]]; then
    COPAW_ROOT=$(echo "$COP_PATH" | sed 's/\/\.molili\/bin\/molili//')
elif [[ "$COP_PATH" == *".molili/bin/molili" ]]; then
    COPAW_ROOT=$(echo "$COP_PATH" | sed 's/\/\.molili\/bin\/molili//')
else
    # 兜底：尝试获取所在目录的父目录
...
echo "Detected molili Root: $COPAW_ROOT"
```
...
如果还是找不到文档，搜索 molili 安装路径下的可用文档内容：
